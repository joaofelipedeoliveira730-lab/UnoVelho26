...
const MAP_PERSONALITY={
  saloon:{label:'Saloon clássico',music:'saloon',decor:'BAR LUZ MADEIRA'},medieval:{label:'Taverna medieval',music:'medieval',decor:'BAR LUZ '},pirate:{label:'Navio pirata',music:'pirate',decor:'PIRATA BAR PORTO'},modern:{label:'Casa moderna',music:'modern',decor:'CAFÉ CASA '},classroom:{label:'Sala de aula',music:'modern',decor:'AULA MATH EDITAR'},neon:{label:'Cidade neon',music:'modern',decor:'CIDADE IDENTIDADE '},forest:{label:'Floresta',music:'forest',decor:'FLORESTA FLORESTA IDENTIDADE'},desert:{label:'Deserto',music:'saloon',decor:'DESERTO FOGO'},ice:{label:'Montanha congelada',music:'modern',decor:'GELO GELO'},space:{label:'Estação espacial',music:'modern',decor:'ESPAÇO ÓRBITA'},math:{label:'Dimensão Matematixa',music:'modern',decor:'∞ IDENTIDADE NÚMEROS'},ceo:{label:'Dimensão CEO',music:'modern',decor:'CEO VALOR MESA'},
  'velho-bar':{label:'Bar do Velho',music:'saloon',decor:'BAR PETISCO LUZ MADEIRA BILHAR'},
  quintal:{label:'Quintal da Vó',music:'modern',decor:'CASA QUINTAL QUINTAL QUINTAL QUINTAL'},
  pier:{label:'Pier do Baralho',music:'modern',decor:'PIER PIER PIER PIER PORTO'}
};
...
function toast(message,type='info',ms=3000){const box=$('#toastContainer');if(!box)return;const el=document.createElement('div');el.className=`toast ${type}`;el.innerHTML=`<span>${type==='error'?'':type==='success'?'OK':'ℹ'}</span><div>${escapeHtml(message).replace(/\n/g,'<br>')}</div>`;box.appendChild(el);setTimeout(()=>el.classList.add('out'),Math.max(300,ms-350));setTimeout(()=>el.remove(),ms);}
...
function renderAchievementsPreview(){const el=$('#achievementPreview');if(!el)return;el.innerHTML=[['RANK','Primeira Vitória'],['FOGO','Sequência de Vitórias'],['GLOBAL','Primeiro Online'],['ITENS','Colecionador']].map(a=>`<div class="achievement-chip"><span>${a[0]}</span><b>${a[1]}</b></div>`).join('');}

const GAME_MODES={uno:{label:'UNO',name:'Uno dos Idosos',icon:'UNO'}};
...
async function selectRoom(code){try{const d=await get('/rooms?mode='+encodeURIComponent(state.selectedGameMode||'uno'));const room=(d.rooms||[]).find(r=>r.code===code);if(!room)return toast('Sala não encontrada.','error');state.roomToJoin=room;$('#joinRoomInfo')&&($('#joinRoomInfo').innerHTML=`<b>${escapeHtml(room.name)}</b><br>${escapeHtml(room.ownerName)} • ${room.players.length}/${room.options.maxPlayers} • ${room.locked?' Com senha':'GLOBAL Aberta'}`);if($('#joinRoomPassword'))$('#joinRoomPassword').value='';show('#joinRoomModal');}catch(e){toast(e.message,'error');}}
...
function renderRoom(room){
  if(!room)return;$('#roomTitle')&&($('#roomTitle').textContent=room.name);$('#roomCodeBadge')&&($('#roomCodeBadge').textContent=room.code);$('#roomOptionsText')&&($('#roomOptionsText').textContent=`${GAME_MODES[room.options?.gameMode]?.icon||'JOGADORES'} ${GAME_MODES[room.options?.gameMode]?.label||'JOGO'} • ${room.players.length}/${room.options.maxPlayers} jogadores • ${room.options.turnSeconds}s`);if($('#btnStartRoom'))$('#btnStartRoom').style.display=String(room.ownerId)===String(state.user.id)&&!room.started?'inline-flex':'none';
  const list=$('#roomPlayers');if(list)list.innerHTML=room.players.map(p=>`<div class="room-player ${String(p.userId)===String(room.ownerId)?'host':''}"><div class="player-avatar">PERFIL</div><div><b>${escapeHtml(p.username)}</b><small>${String(p.userId)===String(room.ownerId)?'CEO Criador':'Jogador'} </small></div><span>${p.connected?'●':'○'}</span></div>`).join('');
  const banner=$('#roomMapBanner');if(banner){banner.className=`room-map-banner map-${mapTheme(room.options.mapId)}`;banner.innerHTML=`<div><span>MAPA MAPA</span><b>${escapeHtml(MAPS.find(m=>m.id===room.options.mapId)?.name||room.options.mapId)}</b></div>`;}
}
...
function showGameIntro(onDone,opts={}){
  ...
  overlay.classList.remove('hidden');overlay.innerHTML=`<div class="game-start-card"><div class="game-start-kicker">${opts.online?'GLOBAL PARTIDA ONLINE':'TREINO PARTIDA'}</div><div id="gameStartNumber" class="game-start-number">5</div><div class="game-start-label">PREPARE A MESA!</div><div id="gameStartDeck" class="game-start-deck" aria-hidden="true">${Array.from({length:10},(_,i)=>`<span style="--d:${i}">${['7','+2','3','COR','9','0','+4','5','REV','2'][i]}</span>`).join('')}</div></div>`;
...
function showSoloMatchmaking(onFound){
  ...
  soloMatchTimer=setTimeout(()=>{clearInterval(tick);overlay.innerHTML=`<div class="game-start-card match-found-card"><div class="game-start-kicker">TREINO OPONENTE ENCONTRADO</div><div class="match-found-icon">OK</div><div class="matchmaking-title">PARTIDA ENCONTRADA!</div><div class="match-found-sub">Preparando a mesa...</div></div>`;vibrate([250,100,250,100,400]);speakMatchFound();Sound.ok();setTimeout(()=>{showGameIntro(onFound,{online:false});},850);},limit);
}
...
// ---------------- SOLO ----------------
function makeDeck(){const d=[];for(const color of COLORS){for(let n=0;n<=9;n++)d.push({id:crypto.randomUUID(),color,value:String(n),type:'number'});d.push({id:crypto.randomUUID(),color,value:'BLOQUEIO',type:'skip'});d.push({id:crypto.randomUUID(),color,value:'REV',type:'reverse'});d.push({id:crypto.randomUUID(),color,value:'+2',type:'draw2'});}for(let i=0;i<4;i++){d.push({id:crypto.randomUUID(),color:'black',value:'COR',type:'wild'});d.push({id:crypto.randomUUID(),color:'black',value:'+4',type:'draw4'});}for(let i=d.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[d[i],d[j]]=[d[j],d[i]];}return d;}
...
function dancePodiumCharacters(entries){
  return entries.map((e,i)=>`<div class="podium-slot place-${i+1}"><div class="podium-confetti">${i===0?'CEO':'IDENTIDADE'}</div><div class="podium-dancer">${characterMarkup(e.avatar||DEFAULT_AVATAR,e.name||'Jogador')}</div><div class="podium-base"><strong>${i+1}º</strong><b>${escapeHtml(e.name||'Jogador')}</b><small>${escapeHtml(e.label||'')}</small></div></div>`).join('');
}
function showPodium(entries){
  const overlay=$('#podiumOverlay');if(!overlay)return;const top=[...entries].slice(0,3);overlay.innerHTML=`<div class="podium-card"><button class="podium-close" id="btnClosePodium" type="button">×</button><span class="pill">RANK PARTIDA ENCERRADA</span><h2>O PÓDIO DO BARALHO!</h2><p>Os campeões estão comemorando!</p><div class="podium-stage">${dancePodiumCharacters(top)}</div><button class="btn btn-primary btn-wide" id="btnPodiumLobby" type="button"> VOLTAR AO LOBBY</button></div>`;overlay.classList.remove('hidden');Sound.win();[0,1,2].forEach(i=>setTimeout(()=>Sound.emote(),i*180));on('#btnClosePodium','click',closePodium);on('#btnPodiumLobby','click',closePodium);}
...
function renderDrawingState(g){
  ...
  const mine=String(g.drawerId)===String(state.user?.id);const title=$('#drawTurnLabel');if(title)title.textContent=mine?'EDITAR SUA VEZ DE DESENHAR':' '+(g.drawerName||'Jogador')+' está desenhando';
  ...
  const feed=$('#drawGuessFeed');if(feed&&g.guesses){feed.innerHTML=g.guesses.map(x=>`<div class="guess-line ${x.correct?'correct':''}"><b>${escapeHtml(x.username)}</b><span>${escapeHtml(x.guess)}</span>${x.correct?'<em>OK ACERTOU</em>':''}</div>`).join('');feed.scrollTop=feed.scrollHeight;}
}
...
function renderDrawingReveal(g){
  ...
  toast(`BUSCA Era: ${g.word||'???'}`,'success',4500);
}
function renderDrawingGuess(g){
  const feed=$('#drawGuessFeed');if(!feed)return;const line=document.createElement('div');line.className=`guess-line ${g.correct?'correct':''}`;line.innerHTML=`<b>${escapeHtml(g.username)}</b><span>${escapeHtml(g.guess)}</span>${g.correct?'<em>OK ACERTOU</em>':''}`;feed.appendChild(line);feed.scrollTop=feed.scrollHeight;if(g.correct)Sound.ok();
}
...
function handleGameAction(action={}){
  if(action.type==='play'){Sound.play();toast(`UNO ${escapeHtml(action.username||'Jogador')} jogou uma carta.`,'info',1200);}
...
function handleGameEmote(m={}){
  ...
  if(target){target.textContent=m.emote||'SORRISO';target.classList.add('show');setTimeout(()=>target.classList.remove('show'),1600);}
}
...
function renderModeGameState(g){
  ...
  const labels={truco:[' TRUCO','Truco do Velho'],checkers:['PEDRA DAMAS','Damas de Botecão'],chess:['PEÃO XADREZ','Xadrez do Bar']};
  const meta=labels[g.mode]||['JOGADORES JOGO','Mesa'];
  ...
  if(g.mode==='truco'){
    const hand=g.hand||[];body.innerHTML=`<div class="online-mode-board truco-online-board"><div class="mode-scorebar">${(g.players||[]).map((p,i)=>`<div class="mode-player-chip"><div class="mode-player-char">${characterMarkup(p.avatar||DEFAULT_AVATAR,p.username)}</div><div><b>${escapeHtml(p.username)}</b><small>${p.points||0} mãos</small></div></div>`).join('')}<div class="truco-bet">VALENDO <b>${g.bet||1}</b></div></div><div class="mode-center-table"><div class="trick-cards">${(g.trick||[]).map(x=>`<div class="playing-card-mini"><b>${escapeHtml(x.card.value)}</b><span>${escapeHtml(x.card.suit)}</span><small>${escapeHtml(x.username)}</small></div>`).join('')||'<div class="mode-empty">UNO Jogue uma carta</div>'}</div></div><div class="mode-hand-row">${hand.map(c=>`<button class="playing-card-big" data-mode-card="${escapeHtml(c.id)}" type="button"><b>${escapeHtml(c.value)}</b><span>${escapeHtml(c.suit)}</span></button>`).join('')}</div><div class="mode-actions"><button class="btn btn-primary" id="modeTruco" type="button">FOGO TRUCO!</button></div></div>`;
  ...
  const isChess=g.mode==='chess';const board=g.board||[];const pieces=isChess?{'r':'TORRE','n':'CAVALO','b':'BISPO','q':'DAMA','k':'REI','p':'','R':'','N':'','B':'','Q':'','K':'','P':''}:{'w':'','W':'CEO','b':'PEDRA','B':'CEO'};
...
function applyMapScene(mapId='map_velho_bar'){
  ...
  const cups=$$('.table-cup',scene); cups.forEach((c,i)=>c.textContent=theme==='quintal'?(i?'QUINTAL':'QUINTAL'):theme==='pier'?(i?'PIER':'PIER'):'BAR');
}
...
const ORIGINAL_CHARACTERS={
  velhinho:{name:'Velhinho',icon:'VELHO',power:'Compra sortuda',req:'Inicial',unlock:true},
  barman:{name:'Barman',icon:'BAR',power:'+1 compra especial',req:'Loja • 650 moedas'},
  rainha:{name:'Rainha da Mesa',icon:'CEO',power:'Aura de sorte',req:'Passe • nível 25'},
  astronauta:{name:'Astronauta',icon:'ESPAÇO',power:'Efeito espacial',req:'Loja • 1.200 moedas'},
  rei:{name:'Rei do Baralho',icon:'UNO',power:'Título dourado',req:'Passe • nível 75'}
};
...
function renderCharactersPage(){
  ...
  const el=$('#charactersCatalog');if(!el)return;el.innerHTML=Object.entries(ORIGINAL_CHARACTERS).map(([id,c])=>{const own=characterOwned(id),sel=(state.profile.avatar.character||'velhinho')===id;return `<button type="button" class="character-card-3d ${own?'unlocked':'locked'} ${sel?'selected':''}" data-character-id="${id}"><div class="character-card-visual">${characterMarkup({...state.profile.avatar,character:id},c.name)}</div><div class="character-card-info"><div><b>${c.icon} ${c.name}</b><span>${own?'DESBLOQUEADO':' BLOQUEADO'}</span></div><small>DUO ${c.power}</small><em>${own?'Toque para usar':c.req}</em></div></button>`;}).join('');
...
function renderFriends(){const el=$('#friendsList');if(!el)return;if(!state.friends.length){el.innerHTML='<div class="empty-state"><span>GRUPO</span><b>Nenhum amigo ainda.</b><small>Use ＋ ADICIONAR para chamar um jogador.</small></div>';return;}el.innerHTML=state.friends.map(f=>`<div class="friend-row"><div class="friend-avatar">PERFIL</div><div class="friend-info"><b>${escapeHtml(f.username)}</b><small>ID: ${escapeHtml(f.id)}</small></div><span class="presence ${escapeHtml(f.status)}">${f.status==='online'?'● ONLINE':f.status==='away'?'● AUSENTE':'● OFFLINE'}</span>${f.status==='online'?`<button class="friend-invite" data-invite-friend="${f.id}" type="button">＋</button>`:'<span></span>'}</div>`).join('');}
...
function renderMail(){const el=$('#mailContent');if(!el)return;if(!state.invites.length){el.innerHTML='<div class="empty-state glass"><span>VAZIO</span><b>Correio vazio.</b><small>Nenhum convite pendente.</small></div>';return;}el.innerHTML=state.invites.map(i=>`<article class="mail-row glass"><div class="mail-icon">JOGADORES</div><div><b>${escapeHtml(i.from_username||'Jogador')}</b><small>Convite para jogar ${escapeHtml(i.mode||'UNO')}.</small></div><button class="btn btn-primary" data-accept-invite="${i.id}" type="button">ACEITAR</button><button class="btn btn-secondary" data-decline-invite="${i.id}" type="button">IGNORAR</button></article>`).join('');}
...
async function toggleMaintenance(){
  ...
  try{
    if(active){
      ...
      if(btn)btn.textContent='MANUTENÇÃO ATIVAR MODO MANUTENÇÃO';
...
function applyMaintenance(paused,message){
  ...
  if(paused){
    ...
    if($('#globalPauseBanner'))$('#globalPauseBanner').textContent='MANUTENÇÃO JOGO EM MANUTENÇÃO';
...
async function openCEOPanel(){
  ...
  try{
    ...
    const btn=$('#btnMaintenance');
    if(btn)btn.textContent=state.maintenance?'▶ LIBERAR O JOGO':'MANUTENÇÃO ATIVAR MODO MANUTENÇÃO';
...
function itemVisualIcon(item){
  ...
  if(cat==='shoes') return '';
  if(cat==='accessory') return id.includes('glasses')?'':id.includes('hat')?'':id.includes('backpack')?'ITENS':'IDENTIDADE';
  if(cat==='effect') return 'COR';
  if(cat==='emote') return '';
  if(cat==='title') return '';
  if(cat==='map') return 'MAPA';
  if(cat==='deck') return 'UNO';
  if(cat==='table') return '';
  return '';
}
...
async function openShop(mode='official'){
  ...
  const grid=$('#shopGrid');
  if(!grid)return;
  grid.innerHTML='<div class="loading glass">LOJA Carregando catálogo...</div>';
  try{
    ...
    grid.innerHTML=list.map(item=>{
      ...
      return `<article class="item-card glass rarity-${escapeHtml(item.rarity||'common')}">
        ...
        <div class="item-buy"><span>MOEDAS ${fmt(item.price)}${Number(item.xp_required)>0?` • ${fmt(item.xp_required)} XP`:''}</span>
        <button class="btn ${ownedNow?'btn-secondary':'btn-primary'} item-buy-btn" data-buy-item="${escapeHtml(item.id)}" type="button" ${canBuy?'':'disabled'}>${ownedNow?'OK POSSUI':locked?' CEO':'COMPRAR'}</button></div></div>
      </article>`;
...
function renderInventoryItems(){
  ...
  if(state.inventoryMode==='achievements'){
    el.innerHTML='<div class="empty-state">RANK Conquistas serão mostradas conforme você vence partidas.</div>';return;
  }
  if(!state.inventory.length){el.innerHTML='<div class="empty-state">ITENS Seu inventário está vazio. Visite a loja!</div>';return;}
...
async function openInventory(mode='items'){
  ...
  navigate('inventory');
  const el=$('#inventoryContent');if(el)el.innerHTML='<div class="loading">ITENS Carregando inventário...</div>';
  try{
  ...
  try{
    ...
    grid.innerHTML=(d.levels||[]).map(r=>{
      ...
      return `<article class="pass-card glass ${unlocked?'unlocked':'locked'} ${done?'claimed':''}">
        <div class="pass-level">NÍVEL ${r.level}</div><div class="pass-reward-icon">${r.itemId?'':r.title?'':'MOEDAS'}</div>
        <b>${r.itemId?escapeHtml(itemName(r.itemId)):r.title?escapeHtml(itemName(r.title)):'Moedas'}</b>
        <small>MOEDAS +${fmt(r.coins)}</small>
        <button class="btn ${done?'btn-secondary':'btn-primary'}" data-pass-level="${r.level}" type="button" ${unlocked&&!done?'':'disabled'}>${done?'OK COLETADO':unlocked?'COLETAR':''}</button>
      </article>`;
    }).join('');
...
function renderCustomCatalog(category){
  ...
  el.innerHTML=ids.map(id=>{
    ...
    return `<button class="custom-item-card ${state.profile.avatar[category]===id?'selected':''} ${canUse?'owned':'locked'}" data-custom-item="${escapeHtml(id)}" data-custom-slot="${escapeHtml(category)}" type="button">
      <div class="custom-photo item-only-photo generated-item-visual"><span>${itemVisualIcon(item||{category:category})}</span><span class="photo-badge">${canUse?'OK USAR':`MOEDAS ${fmt(price)}`}</span></div>
...
async function equipCustomItem(id,slot){
  ...
  if(!owned.has(id)&&state.user?.role!=='CEO'){
    const item=state.items.find(x=>x.id===id); if(item){toast(`Compre ${item.name} na loja por MOEDAS ${fmt(item.price)}.`,'info');openShop('official');}else toast('Item bloqueado.','error');return;
  }
...
/* MATEMATIXA 7.0 — cinematic interaction layer */
(function(){
  const bootCinematic=()=>{
    document.body.classList.add('uv-cinematic-ready');
    const backdrop=document.querySelector('.uv-cinematic-backdrop');
    if(!backdrop)return;
    const move=(e)=>{
      if(window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;
      const x=(e.clientX/window.innerWidth-.5)*2;
      const y=(e.clientY/window.innerHeight-.5)*2;
      backdrop.style.transform=`translate3d(${x*-5}px,${y*-4}px,0)`;
      document.documentElement.style.setProperty('--uv-mx',`${x*8}px`);
      document.documentElement.style.setProperty('--uv-my',`${y*6}px`);
    };
    window.addEventListener('pointermove',move,{passive:true});
  };
  document.addEventListener('DOMContentLoaded',()=>{
    bootCinematic();
    const lore=document.querySelector('#btnLore');
    if(lore) lore.addEventListener('click',()=>{
      const modal=document.querySelector('#loreModal');
      if(modal) modal.classList.remove('hidden');
      try{Sound.ok()}catch{}
    });
    document.querySelectorAll('.big-menu-btn,.rail-btn,.btn,.platform-option').forEach(btn=>{
      btn.addEventListener('pointerdown',()=>{try{Sound.click()}catch{}},{passive:true});
    });
  });
})();
